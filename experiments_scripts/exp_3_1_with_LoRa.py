...
'''
EXPERIMENT 3.1 SCRIPT with LoRa: Done
...
The paper experiment was ran with epoch1 =10 epoch2 =20 and both LRs were 0.001
...
results/experiment_3_1/experiment_3_1_paper_version_LoRA_submitted_conf.json
...
def main():
        ...
        for f, seed in enumerate(config['EXPERIMENT_CONFIG']['seeds']):
                ...
                # Added dataset_C as a proxy to control for forgotten effect (third update)
                dataset_C = tokenized_dataset_t.select(range(sample_sizeA + sample_sizeB, sample_sizeA + sample_sizeA + sample_sizeB))
                ...
                ########################################################################################################
                # FT C Using Full finetuning (as a control to see impact on A => forgot )
                print(f'Fold n. {f} - FT C - control!')

                res = {"epoch_acc_A":[], "epoch_acc_B":[], "acc_A": [], "acc_B": [], 'acc_gen': []} #  B here, will mean C, but it's the fastest way before deadline.
                ...
                for r in range(config['EXPERIMENT_CONFIG']['n_rep']):
                        ...
                        epoch_score_B, epoch_score_A, score_B, score_A= ft_custom(
                                ...
                                batch_size=config['FT_CONFIG']["batch_size"], 
                                dataset_update=dataset_C, #update by putting dataset_C
                                dataset_eval=dataset_A, 
                                out_dir=None, 
                                logging_dir=f"{config['FT_CONFIG']['logging_dir']}",
                                padding_mask=True,
                        ...
                        print(f'FTC scores - A:{score_A} C (control):{score_B} ')
                ...
                ########################################################################################################
                # FT NOT-B USING Full finetuning
                ...
                ########################################################################################################
                # FT NOT-B USING LORA (instead of full finetuning)
                print(f'Fold n. {f} - FT NOT-B')
                ...
                for r in range(config['EXPERIMENT_CONFIG']['n_rep']):
                        ...
                        # Perform FT
                        epoch_score_B, epoch_score_A, score_B, score_A, score_gen= ft_custom_lora(
                                model=ft_modelB, 
                                lr=config['FT_CONFIG']["training_lr_2"], 
                                num_epochs=config['FT_CONFIG']["num_epochs_2"], 
                                batch_size=config['FT_CONFIG']["batch_size"], 
        ...
        filename = os.path.join(config['EXPERIMENT_CONFIG']['results_dir'],
                                f'experiment_3_1_paper_version_LoRA_{timestamp}.json')
