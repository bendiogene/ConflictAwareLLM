...
def main():
        ...
        specs = {'title': config['EXPERIMENT_CONFIG']['title'],
                 ...
                 'n_epochs_A': config['FT_CONFIG']['num_epochs_3'],
        ...
        for f, seed in enumerate(config['EXPERIMENT_CONFIG']['seeds']):
                ...
                dataset_gen = tokenized_dataset_gen.select(range(sample_sizeH + sample_sizeA, sample_sizeH + sample_sizeA + sample_sizeB))
                ...
                tot_neurons = config['EXPERIMENT_CONFIG']['tot_neurons']
                

                fold_results['cfta'] = {}
                fold_results['cftb'] = {}
                fold_results['cft-notb'] = {}

                ########################################################################################################
                # C-FT A

                print(f'Fold n. {f} - C-FTA')
                ...
                epoch_score_A, _, score_A, _, stubborn_number= ft_custom(
                        ...
                        save_model=True,
                        inverse = True,
                        threshold=tot_neurons-config['HISTORICAL_FEATURES_CONFIG']["threshold"][-1],
                        freeze_other_layers=False
                        )

                print(f'Scores - A:{score_A}')

                fold_results['cfta']['acc_A'] = score_A
                fold_results['cfta']['epoch_acc_A'] = epoch_score_A
                ...
                for t in config['HISTORICAL_FEATURES_CONFIG']["threshold"]:
                        ...
                        fold_results['cft-notb'][t] = {'acc_A': [], 'epoch_acc_A': [], 'acc_B':[], 'epoch_acc_B': [], 'acc_gen':[]}

                        
                        ########################################################################################################
                        # C-FT B

                        print(f'Fold n. {f} - C-FTB')

                        # Load ft model
                        ft_modelA = AutoModelForCausalLM.from_pretrained(out_dir_ftA)
                        ...
                        fold_results['cftb'][t]['acc_A']=score_A
                        fold_results['cftb'][t]['acc_B']=score_B
                        ...
                        dataset_A = dataset_A.select(known_facts)
                        ...
                        for r in range(config['EXPERIMENT_CONFIG']['n_rep']):
                                ...
                                print(f'Fold n. {f} - C-FTB NOT B')

                                # Load ft model
                                ft_modelB = AutoModelForCausalLM.from_pretrained(out_dir_ftB)

                                # Perform C-FT
                                epoch_score_B, epoch_score_A, score_B, score_A, score_gen, stubborn_number= ft_custom(
                                        model=ft_modelB, 
                                        lr=config['FT_CONFIG']["training_lr_2"], 
                                        num_epochs=config['FT_CONFIG']["num_epochs_3"], 
                                        batch_size=config['FT_CONFIG']["batch_size"], 
                                ...
                                fold_results['cft-notb'][t]['acc_A'].append(score_A)
                                fold_results['cft-notb'][t]['epoch_acc_A'].append(epoch_score_A)

                                fold_results['cft-notb'][t]['acc_B'].append(score_B)
                                fold_results['cft-notb'][t]['epoch_acc_B'].append(epoch_score_B)

                                fold_results['cft-notb'][t]['acc_gen'].append(score_gen)

                                del ft_modelB
                                gc.collect()
                                torch.cuda.empty_cache()

                        fold_results['cft-notb'][t]['avg_accA'] = sum(fold_results['cft-notb'][t]['acc_A'])/len(fold_results['cft-notb'][t]['acc_A'])
                        fold_results['cft-notb'][t]['avg_accB'] = sum(fold_results['cft-notb'][t]['acc_B'])/len(fold_results['cft-notb'][t]['acc_B'])
                        fold_results['cft-notb'][t]['avg_accGen'] = sum(fold_results['cft-notb'][t]['acc_gen'])/len(fold_results['cft-notb'][t]['acc_gen'])
