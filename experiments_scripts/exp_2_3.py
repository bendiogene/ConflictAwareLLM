...
def main():
        ...
        for f, seed in enumerate(config['EXPERIMENT_CONFIG']['seeds']):
                ...
                for t in config['HISTORICAL_FEATURES_CONFIG']["threshold"]:
                        ...
                        for r in range(config['EXPERIMENT_CONFIG']['n_rep']):
                                ...
                                fold_results['cftb'][t] = {'acc_A':[], 'epoch_acc_A': [], 'acc_B':[], 'epoch_acc_B': []}
                                ...
                                fold_results['cftb'][t]['acc_A'].append(score_A)
                                fold_results['cftb'][t]['epoch_acc_A'].append(epoch_score_A)

                                fold_results['cftb'][t]['acc_B'].append(score_B)
                                fold_results['cftb'][t]['epoch_acc_B'].append(epoch_score_B)

                                del ft_modelA
                                gc.collect()
                                torch.cuda.empty_cache()

                        fold_results['cftb'][t]['avg_accA'] = sum(fold_results['cftb'][t]['acc_A'])/len(fold_results['cftb'][t]['acc_A'])
                        fold_results['cftb'][t]['avg_accB'] = sum(fold_results['cftb'][t]['acc_B'])/len(fold_results['cftb'][t]['acc_B'])
        ...
        filename = os.path.join(config['EXPERIMENT_CONFIG']['results_dir'],
                                f'experiment_2_3_{timestamp}.json')
