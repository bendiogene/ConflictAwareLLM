...
class ParametersHooks:
    ...
    def get_data(self, last_token_idx, transformations, norm_type=None, norm_dim=None, stats=False):
        ...
        grad_values = [self.gradients[layer].cpu().numpy() for layer in self.activations]
        act_values = [self.activations[layer].cpu().numpy() for layer in self.activations]
    ...
    def get_historical_data(self):
        ...
        for layer in layers_list:
            ...
            g_dict['norm'][layer] = torch.abs(g_norm).cpu().numpy()
            a_dict['norm'][layer] = torch.abs(a_norm).cpu().numpy()
    ...
    def get_historical_data_sum(self):
        ...
        for layer in layers_list:
            ...
            g_dict['raw'][layer] = sum_g.cpu().numpy()
            a_dict['raw'][layer] = sum_a.cpu().numpy()
            ...
            g_dict['norm'][layer] = sum_g_norm.cpu().numpy()
            a_dict['norm'][layer] = sum_a_norm.cpu().numpy()
    ...
    def get_top_gradients(self,k,idx):
        ...
        for i,layer in enumerate(layers_list):
            ...
            idxs=np.argsort(np.abs(self.gradients[layer][idx].cpu().numpy()))[-k:]
